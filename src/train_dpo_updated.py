...
def generate_pairs(
    ...
    Processes examples in batches of `batch_size`. For each batch:
      ...
      4. Otherwise, build per-example candidate pair pools (with R/2 cap),
         then sample up to `num_pairs_per_batch` pairs via round-robin across
         examples to ensure every usable example is represented equally.
    ...
    for batch_idx in tqdm(range(num_batches), desc=desc):
        ...
        # Build per-example candidate pair pools (with R/2 cap),
        # and count correct/incorrect only from usable examples (ones
        # that have both correct and incorrect completions).
        example_pools = []
        usable_correct = 0
        usable_incorrect = 0
        for ex in per_example:
            if ex["correct"] and ex["incorrect"]:
                usable_correct += len(ex["correct"])
                usable_incorrect += len(ex["incorrect"])
                ...
                example_pools.append(ex_pairs)

        if not example_pools or usable_correct < min_correct or usable_incorrect < min_incorrect:
            batches_skipped += 1
            continue
        ...
        if pair_mode == "per_example":
            ...
            # Stratified round-robin: cycle through examples so each gets
            # equal representation, maximizing prompt diversity.
            for pool in example_pools:
                random.shuffle(pool)
            sampled = []
            indices = [0] * len(example_pools)
            while len(sampled) < num_pairs_per_batch:
                added = False
                for i, pool in enumerate(example_pools):
                    if indices[i] < len(pool):
                        sampled.append(pool[indices[i]])
                        indices[i] += 1
                        added = True
                        if len(sampled) >= num_pairs_per_batch:
                            break
                if not added:
                    break
